...
// 执行
void Stage::Run(const std::vector<BlockingQueue<ChannelData> *> &in_chans,
                const std::vector<BlockingQueue<ChannelData> *> &out_chans) {
  if (in_chans.size() != this->InChanNum()) {
    std::runtime_error("len(ins) != stage.InChanNum()");
  }

  if (out_chans.size() != this->OutChanNum()) {
    std::runtime_error("len(outs) != stage.OutChanNum()");
  }
  ...
  for (auto &h : this->handlers_) {
    ...
    schedulers.push_back(std::make_unique<HandlerScheduler>(
        h,
        std::vector<BlockingQueue<ChannelData> *>(in_chans.begin() + ii,
                                                  in_chans.begin() + ii + nii),
